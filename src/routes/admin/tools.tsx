...
const emptyTool: Omit<Tool, "id" | "sort_order"> = {
  name: "",
  svg_path: null,
};
...
function ToolsPage() {
  ...
  const saveMutation = useMutation({
    mutationFn: async (data: typeof form) => {
      if (editing) {
        await supabase.from("tools").update(data).eq("id", editing.id);
      } else {
  ...
  const openEdit = (t: Tool) => {
    setEditing(t);
    setForm({ name: t.name, svg_path: t.svg_path });
    setOpen(true);
  };
    ...
    <div>
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="mt-2 text-muted-foreground">{tools.length} tools</p>
        </div>
        ...
        <DialogContent className="max-w-lg">
          ...
          <div className="space-y-4 py-4">
            ...
            <div className="space-y-2">
              ...
              <Textarea
                value={form.svg_path ?? ""}
                onChange={(e) => setForm({ ...form, svg_path: e.target.value || null })}
